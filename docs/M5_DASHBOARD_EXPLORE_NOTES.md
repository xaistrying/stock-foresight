...
## Open threads (resolved as of the shipped change)

Both threads below were open when this session ended; recorded resolution
found by checking the archived change and current code (not left
unresolved as originally written):

- Rename question — **no rename.** The change kept the name
  `vite-react-dashboard-ticker-panel`
  (`openspec/changes/archive/2026-08-12-vite-react-dashboard-ticker-panel`).
- Visual container question — **one continuous panel, not separate
  cards.** `design.md`'s Open Questions deferred this to the
  implementation/design pass; the shipped
  `frontend/src/components/AIInsightPanel/AIInsightPanel.jsx` uses one
  panel with `ai-insight-panel__item` divisions, no separate stat-tile
  cards.
