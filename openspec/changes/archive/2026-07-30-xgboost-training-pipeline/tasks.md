...
- [x] 1.1 Add `xgboost` and `scikit-learn` (or equivalent split/metrics
      utilities) to `backend/requirements.txt`; reinstall and verify
      import works in the backend venv.
- [x] 1.2 Create `backend/data/models/` directory; add it to
      `backend/.gitignore` (matches `backend/data/app.db`'s existing
      not-checked-in pattern).
- [x] 1.3 Confirm all 9 training tickers (TCB, VIB, VHM, VND, MWG, HPG,
...
- [x] 2.1 Implement a function that reads `features` rows for the 9 named
      tickers from SQLite.
- [x] 2.2 Filter to `near_gap = 0 AND target IS NOT NULL` ("clean+labeled"
      rows) per design.md Decision 2.
- [x] 2.3 Assemble the model input feature matrix from the existing
...
- [x] 2.4 Add a data-loading test asserting the ticker column (and any
      ticker-identity encoding) is absent from the assembled feature
      matrix.
- [x] 2.5 Add a data-loading test asserting no row with `near_gap = 1` or
...
- [x] 3.1 Implement pooled fold-boundary computation: shared calendar-date
      cutoffs across all 9 tickers, expanding training window, 4-6 folds
      (design.md Decision 4).
- [x] 3.2 Implement the training-side purge: for each fold boundary at
...
- [x] 3.3 Add a leakage guard test: assert that for every fold, no
      training row's underlying `close[t+5]` date is `>= ` that fold's
      test start date.
- [x] 3.4 Add a test confirming folds are NOT constructed via random
      shuffling — e.g. assert row dates within each fold's train/test
      split are monotonically non-decreasing per ticker.

## 4. Model training

- [x] 4.1 Implement the XGBoost training function with a fixed,
...
- [x] 4.2 Train the final pooled model on the full clean+labeled dataset
...
- [x] 6.1 Decide and implement the persistence mechanism for out-of-fold
      ...
      recent ~60.
- [x] 6.2 If using a new SQLite table: add its `CREATE TABLE` to
      `backend/app/db/schema.py` alongside the existing `ohlcv`/`tickers`/
      `features` tables.
- [x] 6.3 Implement a function to compute a given ticker's hit-rate over
...
- [x] 6.4 Add a test verifying the rolling hit-rate function returns a
      correct value against a small synthetic set of persisted
      predictions with known hits/misses.

## 7. Documentation

- [x] 7.1 Create `docs/MODEL_CARD.md` documenting: training data (9
...
- [x] 7.2 Document, in `docs/MODEL_CARD.md`, the known blind spot from
...
- [x] 8.1 Run the full test suite (`pytest backend/tests`) and confirm all
      new and existing tests pass.
- [x] 8.2 Run the training + backtest pipeline end-to-end against the
...
- [x] 8.3 Confirm no code in this change frames results as investment
      advice, predicted returns, or performance guarantees (Rule 6) —
      spot-check `docs/MODEL_CARD.md` language specifically.
- [x] 8.4 Update `openspec/config.yaml`'s milestone status line to mark M3
