...
- [ ] 1.1 In `backend/app/main.py`'s `lifespan` handler, load the persisted
...
- [ ] 1.2 Add a test confirming `TestClient(app)` (used as a context manager,
      ...
      model path constant).
- [ ] 1.3 Add a test confirming a valid model file loads successfully at
      startup and `app.state.model` is populated and reusable across
      requests without reloading.

## 2. Prediction endpoint — happy path

- [ ] 2.1 Create `backend/app/api/predictions.py` with a router exposing
      `GET /tickers/{ticker}/prediction`.
- [ ] 2.2 Implement the query for the ticker's most recent `features` row
...
- [ ] 2.3 When the row exists and `near_gap = 0`, run it through
      ...
      Decision 6).
- [ ] 2.4 Register the new router in `backend/app/main.py` alongside the
      existing `tickers_router`.
- [ ] 2.5 Add a test: clean latest row -> `200`, `status: "ok"`,
...
- [ ] 3.0 Add a migration step (in `init_db()` or a dedicated migration
...
- [ ] 3.1 Add a `features_computed` column to the `tickers` table in
...
- [ ] 3.2 Move the existing `UPSERT_TICKER` call to after the
      feature-computation try/except resolves; include
      `features_computed` in that same upsert. No second write.
- [ ] 3.3 Add/update a `ticker_ingestion` test confirming `features_computed`
...
- [ ] 4.0 Implement the gating checks in this fixed order (design.md
...
- [ ] 4.1 Return `404` when the ticker has zero `features` rows (check 2,
...
- [ ] 4.2 Return a `5xx` status when the ticker's `tickers` row has
...
- [ ] 4.3 Add a test for the `features_computed = 0` -> `5xx` path, distinct
...
- [ ] 4.4a `features_computed IS NULL` (ticker loaded only before this
...
- [ ] 4.4b Same setup, latest row `near_gap = 1` -> falls through check 1,
      responds `status: "near_gap"` as normal.

## 5. Prediction endpoint — near_gap handling

- [ ] 5.1 When the latest `features` row has `near_gap = 1`, return `200`
      with `{"ticker", "as_of", "status": "near_gap"}` and no
      `predicted_log_return` field — do not call the model at all in this
      path.
- [ ] 5.2 Confirm no fallback/walk-back query to an older `near_gap = 0` row
      exists anywhere in this path (design.md Decision 4).
- [ ] 5.3 Add a test: latest row `near_gap = 1` -> `200`, `status:
      "near_gap"`, `predicted_log_return` absent from the response body.
- [ ] 5.4 Add a test: latest row `near_gap = 1` but an older row for the same
...
- [ ] 6.1 Run `pytest backend/tests` and confirm the full suite passes,
      including all new tests from sections 1-5.
- [ ] 6.2 Manually verify with `uvicorn app.main:app --reload --app-dir
