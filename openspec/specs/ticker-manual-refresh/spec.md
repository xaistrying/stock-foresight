# ticker-manual-refresh

## Purpose

Let a user force a re-fetch of a ticker that is already loaded, rather than
waiting for the next automatic staleness check, and make the ticker's last
load time visible so the refresh action has a legible reason attached to
it. Reuses the existing `POST /tickers/{ticker}/load` endpoint and the
existing per-status load messaging; adds no new backend endpoint.
